      ...
      >
        <View className="space-y-3">
          ...
          <View className="flex-row flex-wrap">
            {['damaged_product', 'wrong_product', 'expired_product', 'missing_items', 'quality_issues', 'other'].map((reason) => (
              <Button
                key={reason}
                title={reason.replace('_', ' ')}
                onPress={() => setReplacementReason(reason)}
                variant={replacementReason === reason ? 'primary' : 'outline'}
                size="small"
                className="mr-2 mb-2"
              />
            ))}
          ...
          <TextInput
            ...
            className="border border-gray-200 rounded-lg p-2 h-20 text-sm"
            multiline
          />
          ...
          <View className="flex-row">
            <Button
              title="Normal"
              onPress={() => setReplacementPriority('normal')}
              variant={replacementPriority === 'normal' ? 'primary' : 'outline'}
              className="mr-2"
            />
            <Button
              title="High"
              onPress={() => setReplacementPriority('high')}
              variant={replacementPriority === 'high' ? 'primary' : 'outline'}
            />
          </View>

          {replacementError && (
            <Text className="text-red-500 text-sm">{replacementError}</Text>
          )}

          <Button
            title={isRequestingReplacement ? 'Submitting...' : 'Submit Request'}
      ...
      >
        <View className="space-y-4">
          ...
          <TextInput
            ...
            maxLength={6}
            className="border border-gray-200 rounded-lg p-3 text-center text-lg tracking-widest"
          />

          {otpError && (
            <Text className="text-red-500 text-sm">{otpError}</Text>
          )}
      ...
      >
        <View className="items-center py-4">
          {qrCodeUrl ? (
            ...
            <Text className="text-red-500">{qrError}</Text>
          ) : (
            <ActivityIndicator size="large" color="#10B981" />
          )}
        </View>
